...
class AE9c(torch.nn.Module):
    def __init__(self, dropout):
        ...
        self.decoder_conv2 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=3, out_channels=64, kernel_size=9, stride=1, padding=4),
            nn.BatchNorm2d(64),
            nn.ReLU(),

            nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=9, stride=1, padding=4),
            nn.BatchNorm2d(64),
            nn.ReLU(),

            nn.ConvTranspose2d(in_channels=64, out_channels=3, kernel_size=9, stride=1, padding=4),
            nn.Sigmoid()
        )
